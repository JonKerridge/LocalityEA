...
class LocalitySOPPopulation implements  LocalityPopulation{
  ...
  List <LocalitySOPIndividual> population // to hold the list of individuals that form the population
  List evaluateData   // the data structure used to hold the fitness evaluation data if required

  LocalitySOPPopulation(
      int individuals,   // active population plus the space for reproduction offspring
      int geneLength,    // the length of each individuals chromosome
      int crossoverPoints, // the EVEN number of points used to break the chromosome when doing reproduction
      int maxGenerations,  // maximum number of iterations before termination
    ...
    for (i in 0 ..< individuals){
      population << new LocalitySOPIndividual(geneLength)
    }
  }

  static def extractParts(Integer start, Integer end, LocalitySOPIndividual source){
...
//    println "\nChild $child "
  ...
  void reproduce(int parent1, 
                 ...
                 int child2, 
                 Random rng) {
    List<Integer> randoms = [2]  // first flexible city is in location 2 of route (chromosome)
...
//    println "randoms = $randoms, \np1 = ${population[parent1].chromosome}, \np2 = ${population[parent2].chromosome}"
...
//    println "parts 0a: $partsOf1, $partsOf2"
    doMultiPointCrossover(partsOf1, partsOf2, population[child1], crossoverPoints, geneLength)
//    println "randoms = $randoms, parts 0: pnt1= ${population[parent1].chromosome},  p1= $partsOf1, " +
...
//    println "parts 1a: $partsOf1, $partsOf2"
    doMultiPointCrossover(partsOf1, partsOf2, population[child2], crossoverPoints, geneLength)
